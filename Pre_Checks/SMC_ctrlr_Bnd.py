...
class controller:
    def __init__(self):
        ...
        self.radii = 0.2    #radius to track

        self.ang = np.zeros((self.dim, 1))      # Euler angles (x1)
        self.prev_len = 7
        self.qu = deque(maxlen= self.prev_len)
        ...
        '''om1=1.96, om2=9.4, om3=1, eps=4.7, eta=0.46, gama=0.5 with bounds Iris Gazebo'''
        
        '''5,20,35,10,20,0.1 good with sinusoidal disturbance, Sigma btw (-3,8)     Tsalla
        5,20,35,10,72,0.5 both disturb, Adap gain along with disturb, Sigma btw (-3,8) (1,15)'''
        '''{3,9,42,10,20,0.5}, {4,9,80,16,30,0.5}'''
        '''om1c = 2.8 ; om2c = 9 ; gamac = 25
        eta = 10 ; rho = 20'''
        self.om1 = 3*np.eye(self.dim)
        self.om2 = 9*self.om1
        self.Gama = 42*np.eye(self.dim)
        self.eta = 10
        self.rho = 20
        self.eps = 0.5
        ...
        # self.J = np.diag([0.029,0.029,0.055])
        # self.J = np.diag([0.029125,0.029125,0.055225])  # Polar MOI in body frame ------- Iris
        self.J = np.diag([0.009, 0.009, 0.018])  # Polar MOI in body frame ----- Tsalla
    ...
    def controller(self, states, freq):  
        ...
        self.qu.append(self.ang)
        if(self.t==0.0):
            self.ang_dot = self.qu[0]/self.h
        elif((self.t/self.h)<=self.prev_len):
            self.ang_dot = (self.qu[-1] - self.qu[-2])/self.h
        else:
            self.ang_dot = (self.qu[-1] + 2*self.qu[-2] - 2*self.qu[-4] - self.qu[-5])/(8*self.h)
        ...
        # self.ang_ref = np.array([[self.radii*np.sin(self.t)], [0.0], [0.2]])
        # self.ang_dot_ref = np.array([[self.radii*np.cos(self.t)], [0.0], [0.0]])
        # self.ang_ddot_ref = np.array([[-self.radii*np.sin(self.t)], [0.0], [0.0]])
        ...
        final = {'time':self.t, 'ang_vel':ang_dot_pub.tolist(), 'ang':ang_pub.tolist(), 'thrust':thrust_norm, 'torq':Tau.tolist(), 'kbar':out, 'sigma':sigma.tolist(), \
                 'a_ref':self.ang_ref.tolist(), 'adot_ref':self.ang_dot_ref.tolist(), 'TwistStat':states.tolist() , 'err1':kai1_inst.tolist(), 'disturb':dis.tolist()}

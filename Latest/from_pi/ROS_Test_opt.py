...
def main():
    ...
    # act_sp_pub = rospy.Publisher('mavros/actuator_control', ActuatorControl, queue_size=20)
    ...
    while(not rospy.is_shutdown() ) :
        if(time.perf_counter()-t1 <= 3):
            ...
            # #### ActuatorControl msg
            # act.group_mix = 0
            # torq_norm = final_op.get('torq_norm')
            # act.controls[0], act.controls[1], act.controls[2], act.controls[3] = torq_norm[0], torq_norm[1], torq_norm[2], final_op.get('thrust')
            # act.header.stamp = rospy.Time.now()
            # act_sp_pub.publish(act)
            ...
            template = "{{'Sys_time': {}, 'Ctrl_time': {}, 'angle(deg)':{}, 'torq':{}, 'sigma': {}, 'Kbar': {},  'Const1':{},  'Const2':{} }}".format(
                (time.perf_counter()-t1), final_op['time'], angle, final_op['torq'], final_op['sigma'], final_op['kbar'], final_op['Const1'], final_op['Const2'])          
            rospy.loginfo(template)
            # if(any(np.abs(item) > 1 or np.isnan(item) for item in rates)):
            if(any(np.abs(item)> 0.2 or np.isnan(item) for item in torq)):
